...
class MydomainSpider(SitemapSpider):
    ...
    def parse_for_sale(self, response):
        ...
        for home in homes:
            ...
            items["price"] = int(home.css("[class^=\"PropertyPrice_price_\"]::text").get())
            ...
            yield items
        total=int(response.css("[class^=\"ResultsCount_resultsCount_\"] p span::text").get())
        ...
        if  total- self.index_number >= self.increment:
